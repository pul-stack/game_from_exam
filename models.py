...
class Abilities:
    ...
    def get_ability(enemy):
        """Назначает врагу случайную способность с учётом шанса выпадения.

        Возможные способности:
            - strong (10%): +30% к HP и урону, красная рамка.
            - fast (10%): +40% к скокрости, синяя рамка.
            - monetary (15%): +40% к награде за убийство, жёлтая рамка.
            - None (65%): без способности, стандартный цвет.

        Изменяет атрибуты переданного enemy напрямую (health, speed, give_money,
        color) в зависимости от выпавшей способности.

        Args:
            enemy (Characters): Объект врага, котрому назначена способность.
        """
...
class Missile:
    ...
    def update(self):
        """Обновляет положение снаряда за один кадр.

        Реализуется самонаводящийся снаряд: каждый кадр пересчитывает
        вектор движения к текущей позиции цели. Если цель побеждена
        или отсутствует - снаряд помечается как not alive и исчезает.

        При достижении цели (расстояние меньше скрости) наносит урон
        и помечается как not alive. Иначе - перемещается по вектору
        к цели с заданной скороостью.
        """
...
class Explosion:
    ...
    def update(self):
        """Обновляет анимацию взрыва.

        Каждые 5 кадров переключаются кадр анимации. Всего 4 кадра.
        После того, как все кадры проходят, то снаряд помечается как
        not alive, чтобы убрать его из списка.
        """
        self.timer += 1
        if self.timer >= 5:
            self.timer = 0
